...
public class UpdateCategoryCommandHandler : IRequestHandler<UpdateCategoryCommand, ErrorOr<CategoryEntity>>
{
    ...
    private void UpdateSubCatgories(CategoryEntity category, UpdateSubCategoryCommand[] subCategories)
    {
        ...
        foreach (var subCategory in subCategories)
        {
            ...
            {
                //Update
                var existingSubCategory = category.SubCategories.FirstOrDefault(x => x.Id == subCategory.Id);
...
internal sealed class UpdateCategoryCommandValidator : AbstractValidator<UpdateCategoryCommand>
{
    ...
    private async Task<bool> BeUniqueName(UpdateCategoryCommand request, string name, CancellationToken cancellationToken)
    {
        return !await _context.Categories
            .AnyAsync(x => x.UserId == request.UserId && x.Id != request.CategoryId && x.Name == name, cancellationToken);
    }
}